...
function assertResponseIsJSON(res) {
  invariant(res.isJSON(), `The JSON:API response contains an invalid content type ${res.headers['content-type']}.`);
}

function assertResponseIsFound(res) {
  invariant(res.status !== 404, `The JSON:API resource was not found.`);
}
